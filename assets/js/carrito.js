...
const CartPage = {
  ...
  _itemHTML(item) {
    ...
    const imgHtml = item.image
      ? `<img src="${item.image}" alt="${item.name}" style="width:100%;height:100%;object-fit:cover;border-radius:14px;">`
      : (item.emoji || '🧸');

    return `
      <div class="cart-card" data-id="${item.id}">
        <div class="cart-card-img" style="overflow:hidden;display:flex;align-items:center;justify-content:center;">${imgHtml}</div>
  ...
  async changeQty(id, delta) {
    await window.toyCart.updateQty(id, delta);
    this.render();
  },
  ...
  async setQty(id, qty) {
    ...
    if (diff === 0) return;
    await window.toyCart.updateQty(id, diff);
    this.render();
  },
  ...
  async removeItem(id) {
    ...
    if (el) {
      ...
      setTimeout(async () => {
        await window.toyCart.remove(id);
        this.render();
      }, 220);
    } else {
      await window.toyCart.remove(id);
  ...
  _parsePrice(price) {
    ...
    if (!price) return 0;
    let str = String(price).trim().replace(/[$\s]/g, '');
